...
# Fallback city names for airports where Aviationstack returns no city field
IATA_CITY_FALLBACK = {
    "GRU": "São Paulo",
    "GIG": "Rio de Janeiro",
    "CGH": "São Paulo",
...
def _get_city_name(destination: str) -> str:
    """Return full city name for a city code."""
    code = destination[:3].upper()
    if code in CITY_DB:
        return CITY_DB[code]["name"]
    if code in IATA_CITY_FALLBACK:
        return IATA_CITY_FALLBACK[code]
    ap = _lookup_airport_aviationstack(code)
    if ap and ap.get("city"):
        return ap["city"]
    if ap and ap.get("airport_name"):
        # Try to extract city from airport name as last resort
        name = ap["airport_name"]
        # Simple heuristic: take first word if it's not "Aeroporto", "Airport", etc.
        first = name.split()[0]
        if first.lower() not in {"aeroporto", "aeropuerto", "airport", "international", "regional", "municipal"}:
            return first
    return code
...
def build_forecast(origin: str, destination: str, start_date: date,
    ...
    destination_name = _get_city_name(destination)
